 ...
 * @param {string} activePage - The name of the currently active page (e.g., 'feed.html').
 * @param {boolean} onIndexPage - True if the current page is the root index.html.
 * @returns {string} The sidebar HTML string.
 */
export function createSidebar(activePage, onIndexPage = false) {
    // CORREÇÃO: O prefixo agora aponta para 'front/pages/' quando na página inicial.
    const pathPrefix = onIndexPage ? 'front/pages/' : '';

    const links = [
        { href: 'feed.html', title: 'Início', icon: 'fa-house-chimney', name: 'Início' },
        { href: 'search.html', title: 'Explorar', icon: 'fa-compass', name: 'Explorar' },
        { href: 'comunidade.html', title: 'Comunidades', icon: 'fa-users', name: 'Comunidades' },
        { href: 'notificacoes.html', title: 'Notificações', icon: 'fa-bell', name: 'Notificações' },
        { href: 'chat.html', title: 'Mensagens', icon: 'fa-paper-plane', name: 'Mensagens' },
        { href: 'game.html', title: 'Jogos', icon: 'fa-gamepad', name: 'Jogos' },
        { href: 'profile.html', title: 'Perfil', icon: 'fa-user-astronaut', name: 'Perfil' }
    ];

    const navLinks = links.map(link => `
        <a href="${pathPrefix}${link.href}" class="nav-link ${activePage === link.href ? 'active' : ''}" title="${link.title}">
    ...
    return `
        <aside class="sidebar">
            <div class="sidebar-header">
                <a href="${pathPrefix}feed.html" class="logo">
...
/**
 * Creates the HTML for the music player (Hybrid Remake V4).
 * @param {boolean} onIndexPage - True if the current page is the root index.html.
 * @returns {string} The music player HTML string.
 */
export function createMusicPlayer(onIndexPage = false) {
    return `
